...
def visualize_network_interactive(report):
    ...
    for entry in report:
        ...
        for port, info in ports.items():
            ...
            if vuln_flag:
                vulns_text = "<br><b>Vulnerabilities:</b><br>" + "<br>".join(
                    [f"{cve.get('id')}: {cve.get('summary')}" for cve in vulns]
                )
            else:
                vulns_text = "No known vulnerabilities found."

            tooltip = f"Port: {port}<br>Banner: {banner}<br>{vulns_text}"

            net.add_node(port_node_id, label=str(port), title=tooltip, color=color, shape="square", size=15)
